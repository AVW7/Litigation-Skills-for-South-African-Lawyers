---
type: moc
cssclass: home-page
tags: [moc, home, landing]
---

# ⚖️ South African Litigation Skills

> *Master the Art of Courtroom Advocacy*
> Based on Chris Marnewick SC's Definitive Guide

---

> [!tip] Welcome
> This interactive vault transforms *Litigation Skills for South African Lawyers* into a connected knowledge base for efficient study and quick reference.

---

## 📊 Quick Stats

|          |        |             |          |
| :------: | :----: | :---------: | :------: |
|  **29**  | **4**  |    **6+**   |  **∞**   |
| Chapters | Phases | Legal Tests | Practice |

---

## 🏛️ The Four Phases of Litigation

### I. Professional Conduct

> **Chapters 1–3** | Client relationships, ethics, alternatives

### II. Pleadings & Drafting

> **Chapters 4–10** | Summons, pleas, applications

### III. Trial Preparation

> **Chapters 11–15** | Evidence, research, strategy

### IV. Trial & Advocacy

> **Chapters 16–28** | Opening, examination, argument

---

## 📂 Quick Access

| Index            | Description                   |
| ---------------- | ----------------------------- |
| [[All Lectures]] | Complete chapter walkthroughs |
| [[All Terms]]    | Legal concepts & definitions  |
| [[All Formulas]] | Tests & requirements          |
| [[Dashboard]]    | Visual study tools            |

---

## 📊 Visual Tools

| Visual                          | Purpose                     |
| ------------------------------- | --------------------------- |
| `burden_of_proof_visual.svg`    | Civil vs Criminal standards |
| `pleadings_flow_visual.svg`     | Document exchange timeline  |
| `motion_application_visual.svg` | Affidavit sequence          |

> [!abstract] Open the full dashboard in your browser for the best interactive experience.

---

## 🧠 Key Concepts

**Facta Probanda** = Material facts you *must prove* (plead these)

**Facta Probantia** = Evidence that proves facts (do NOT plead)

**The Cross-Exam Duty** = Put your version to the witness on contradicted points

**The Prayer** = Every pleading needs clear relief sought

---

## 🎯 Start Here

- **New?** → [[L00 - Introduction & Prefaces]]
- **Need a concept?** → [[All Terms]]
- **Visual learner?** → Open `Assets/study-dashboard.html`
- **Prepping for trial?** → [[L17 - Examination-in-chief]]

---

> [!quote] *"The Motion Court is a paper court. But it is no paper tiger!"* — Chris Marnewick SC

---

> [!note] Plugins Used
> Dataview, Excalidraw, MindElixir, Style Settings

---

Vault v2.0 | Enhanced Visual Edition
